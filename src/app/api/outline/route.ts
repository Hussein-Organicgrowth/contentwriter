...
type LanguageCode = 'en-US' | 'en-GB' | 'es' | 'fr' | 'de' | 'it' | 'pt' | 'nl' | 'pl' | 'sv' | 'da' | 'no' | 'fi';
type CountryCode = 'US' | 'GB' | 'CA' | 'AU' | 'DE' | 'FR' | 'ES' | 'IT' | 'NL' | 'SE' | 'NO' | 'DK' | 'FI' | 'PL' | 'BR' | 'MX';
...
async function fetchAndParseUrl(url: string): Promise<{ headings: string[], content: string } | null> {
  try {
...
async function getCompetitorContent(keyword: string, targetCountry: CountryCode): Promise<Competitor[]> {
  try {
...
export async function POST(req: Request) {
  try {
    const { keyword, title, language = 'en-US', targetCountry }: { 
      keyword: string; 
      title: string; 
    ...
    const competitorAnalysis = competitors.map((comp, index) => `
      ...
      messages: [
        {
          ...
          ONLY OUTPUT THE OUTLINE IN THE FOLLOWING FORMAT:
          1. Section 1
          a. Subsection 1
          b. Subsection 2
          c. Subsection 3
          ...
          KEEP THE OUTLINE TO MAX 4-5 SECTIONS AND SUBSECTIONS EACH.`
        },
        {
          role: "user",
    ...
    const outline = completion.choices[0]?.message?.content?.trim().split('\n') || [];
