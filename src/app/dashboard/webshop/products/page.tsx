...
export default function ProductsPage() {
	...
	const fetchProducts = async (companyName: string) => {
		try {
			setIsLoading(true);
			// Fetch products, pending descriptions, and published status in parallel
			...
			if (publishedResponse.ok) {
				...
				if (
					...
					console.log(
						"Found published products array with length:",
						publishedData.publishedProducts.length
					);

					// Create a set of published product IDs
					const publishedSet = new Set<string>();

					// Update products with published status
					const productsWithPublishedStatus = productsData.products.map(
						(product: ShopifyProduct) => {
							...
							const publishedInfo = publishedData.publishedProducts.find(
								(p: any) => String(p.productId) === productIdStr
							);

							if (publishedInfo) {
								console.log("Found published info for product:", productIdStr);
					...
					// Update the published products set
					console.log(
						"Setting published products set with size:",
						publishedSet.size
					);
					setPublishedProducts(publishedSet);
					productsData.products = productsWithPublishedStatus;
				} else {
					console.log(
						"No published products array found or it's not an array:",
						publishedData
					);
				}
			} else {
			...
			if (pendingResponse.ok) {
				...
				// If no pending descriptions, still set the products
				...
				<DialogContent className="sm:max-w-[425px]">
					<DialogHeader>
						...
						<DialogDescription>
							...
							will remove your connection settings and you'll need to reconnect
							to access your products again.
						</DialogDescription>
					</DialogHeader>
